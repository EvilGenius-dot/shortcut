...
disable_autostart() {
    ...
    if [ "$(command -v systemctl)" ]; then
        ...
        sudo sed -i '/\/root\/sparks\/sparks\ &/d' /etc/rc.local
    fi
...
installapp() {
    ...
    wget -P $PATH_RUST "${DOWNLOAD_HOST}/${ORIGIN_EXEC}" -O "${PATH_RUST}/${PATH_EXEC}" --no-check-certificate 1>/dev/null
