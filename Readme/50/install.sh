...
VERSION="4.1.3"
DOWNLOAD_HOST="https://github.com/EvilGenius-dot/shortcut/raw/main/Readme/50/linux"
ORIGIN_EXEC="aibb-${VERSION}"

SERVICE_NAME="aibbservice"
...
disable_autostart() {
    ...
    if [ "$(command -v systemctl)" ]; then
        ...
        sudo sed -i '/\/root\/aibb\/aibb\ &/d' /etc/rc.local
    fi
