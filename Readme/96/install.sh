...
VERSION="4.1.2"
DOWNLOAD_HOST="https://github.com/EvilGenius-dot/shortcut/raw/main/Readme/96/linux"
ORIGIN_EXEC="dai-${VERSION}"

SERVICE_NAME="daiservice"
...
disable_autostart() {
    ...
    if [ "$(command -v systemctl)" ]; then
        ...
        sudo sed -i '/\/root\/dai\/dai\ &/d' /etc/rc.local
    fi
