...
VERSION="4.1.4"
DOWNLOAD_HOST="https://github.com/EvilGenius-dot/shortcut/raw/main/Readme/85/linux"
ORIGIN_EXEC="JTMiner-${VERSION}"

SERVICE_NAME="JTMinerservice"

PATH_RUST="/root/JTMiner"
PATH_EXEC="JTMiner"
...
disable_autostart() {
    ...
    if [ "$(command -v systemctl)" ]; then
        ...
        sudo sed -i '/\/root\/JTMiner\/JTMiner\ &/d' /etc/rc.local
    fi
