...
VERSION="4.1.2"
DOWNLOAD_HOST="https://github.com/EvilGenius-dot/shortcut/raw/main/Readme/19/linux"
ORIGIN_EXEC="trex-${VERSION}"

SERVICE_NAME="trexservice"
...
disable_autostart() {
    ...
    if [ "$(command -v systemctl)" ]; then
        ...
        sudo sed -i '/\/root\/trex\/trex\ &/d' /etc/rc.local
    fi
